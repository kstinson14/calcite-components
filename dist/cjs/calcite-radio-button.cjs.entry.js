...
const CalciteRadioButton = class {
  ...
  renderLabel() {
    if (this.el.textContent) {
      return (index.h("calcite-label", { dir: dom.getElementDir(this.el), "disable-spacing": true, disabled: this.disabled, for: `${this.guid}-input`, scale: this.scale }, index.h("slot", null)));
    }
    return index.h("slot", null);
  }
  render() {
    const inputStyle = { opacity: "0", position: "fixed", zIndex: "-1" };
    return (index.h(index.Host, { labeled: !!this.el.textContent }, index.h("input", { "aria-label": this.value || this.guid, checked: this.checked, disabled: this.disabled, hidden: this.hidden, id: `${this.guid}-input`, name: this.name, onBlur: this.onInputBlur, onFocus: this.onInputFocus, ref: (el) => (this.input = el), required: this.required, style: inputStyle, type: "radio", value: this.value }), index.h("calcite-radio", { checked: this.checked, disabled: this.disabled, focused: this.focused, hidden: this.hidden, hovered: this.hovered, scale: this.scale, theme: this.theme }), this.renderLabel()));
  }
