...
const CalciteRadioButton = class {
  ...
  renderLabel() {
    if (this.el.textContent) {
      return (h("calcite-label", { dir: getElementDir(this.el), "disable-spacing": true, disabled: this.disabled, for: `${this.guid}-input`, scale: this.scale }, h("slot", null)));
    }
    return h("slot", null);
  }
  render() {
    const inputStyle = { opacity: "0", position: "fixed", zIndex: "-1" };
    return (h(Host, { labeled: !!this.el.textContent }, h("input", { "aria-label": this.value || this.guid, checked: this.checked, disabled: this.disabled, hidden: this.hidden, id: `${this.guid}-input`, name: this.name, onBlur: this.onInputBlur, onFocus: this.onInputFocus, ref: (el) => (this.input = el), required: this.required, style: inputStyle, type: "radio", value: this.value }), h("calcite-radio", { checked: this.checked, disabled: this.disabled, focused: this.focused, hidden: this.hidden, hovered: this.hovered, scale: this.scale, theme: this.theme }), this.renderLabel()));
  }
